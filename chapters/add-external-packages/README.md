...
选用诸如 .NET 这种成熟的技术栈，最大的优势之一，就是第三方软件包和插件的生态系统非常庞大。正如其它的软件包系统(npm, Maven, RugGems)一样，你可以下载并安装 .NET 软件包，以获取帮助，来完成几乎任何可想象的任务和难题。
...
截止日期一栏所展示的日期，其格式对机器来说很方便（称为 ISO 8601），但是人类看着就不爽了。如果看到的是“还剩 X 天”这样，不是更好吗？你完全可以自己写代码，把一个日期转换成人性化的字符串，只是幸好，还有个便捷的方法。

NuGet 上的软件包 Humanizer (https://www.nuget.org/packages/Humanizer) 解决了这种问题，它提供的方法可以“人性化(humanize)” 或者说改写几乎任何东西：日期、时间、时长、数字等等。它是个很好很有用的开源项目，根据 MIT 许可证发布。
...
> 使用 `dotnet new mvc` 创建的项目文件包含一条指向`Microsoft.AspNetCore.All`软件包的引用，这是个便捷的“元软件包(metapackage)”，它引用了你在特定项目中所需的所有其它 ASP.NET Core 软件包。这样，你就不必再项目文件里添加数百个软件包的引用了。
